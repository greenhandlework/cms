 <div class="post-list" id="postList1">  
  ...
  <input type="hidden" name="order_date11" id="order_date11" value="<?php if(isset($order_date)){ echo $order_date;} ?>">
<input type="hidden" name="order_status11" id="order_status11" value="<?php if(isset($order_status)){ echo $order_status;} ?>">
<input type="hidden" name="search11" id="search11" value="<?php if(isset($search)){ echo $search;} ?>">
...
// alert(page_num);
     ...
     var p = page_num;
        var order_date = $('#order_date11').val();
        var order_status = $('#order_status11').val();
        var search = $('#search11').val();
       
        $.ajax({
            type: 'POST',
            url: '<?=ADMIN_PATH.'order_details/ajaxPaginationData_filter/'?>'+page_num,
            data:{page:p,order_date:order_date,order_status:order_status,search:search},
            // beforeSend: function () {
            //     $('.loading').show();
            // },
