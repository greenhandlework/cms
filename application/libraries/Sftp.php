...
class SFTP {
    ...
    public function connect($config = array())
    {
        ...
        if ($this->method == 'key')
        {
            if (!$this->_login_keys())
            {
                if ($this->debug == TRUE)
                {
                    $this->_error('sftp_unable_to_login key');
                }
                return FALSE;
            }       
        } 
        ...
        else
        {
            if (!$this->_login_pass())
            {
                if ($this->debug == TRUE)
                {
                    $this->_error('sftp_unable_to_login pass');
                }
                return FALSE;
            }               
        }
    ...
    {
		//print_r($this->passphrse);
        if ($this->passphrse != '') 
        {
		echo $this->conn_id; echo '<br>';
		echo $this->username; echo '<br>';
		//echo $this->passphrse; echo '<br>';
		
		echo @ssh2_auth_pubkey_file($this->conn_id, $this->username, $this->pubkeyfile, $this->prikeyfile,$this->passphrse);
		exit;
